//
//  SessionEndTimeViewController.swift
//  Schedulo
//
...
class SessionEndTimeViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        timePicker.selectRow(session.time.end.hour, inComponent: 0, animated: false)
        timePicker.selectRow(session.time.end.minute, inComponent: 1, animated: false)
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        if component == 0 {
            session.time.end.hour = row
        } else {
            session.time.end.minute = row
        }
    }
}
